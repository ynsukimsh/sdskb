...
export async function POST(request: NextRequest) {
  try {
    ...
    const config = body.structure !== undefined ? { structure: body.structure } : body

    if (!config || !Array.isArray(config.structure)) {
      return NextResponse.json(
        { success: false, error: 'Invalid config: expected { structure: [...] }' },
        { status: 400 }
      )
    }
