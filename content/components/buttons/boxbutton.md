---
name: BoxButton
description: |-
  가장 중요한 명령을 실행 시킬 때 사용됩니다.
  한 화면에서 전체에 해당하는 CTA 버튼 성격으로 활용 가능합니다.
  목적에 따라 scroll을 해도 화면에 플로팅하여 고정할 수 있습니다.
  필요에 따라 레이블의 좌측에 아이콘을 옵션으로 제공할 수 있습니다.
figma_url: https://www.figma.com/design/S5GcFD0Q0SNJqsQBks0qgz/%F0%9F%9F%A0-SOLID---Core-UI-Kit-2.1?node-id=23-287181&t=Z22cdq1VjHfcbw5i-4
figmaLink: https://www.figma.com/design/S5GcFD0Q0SNJqsQBks0qgz/%F0%9F%9F%A0-SOLID---Core-UI-Kit-2.1?node-id=23-287181&t=vI1LxiuhpkYXfXsB-11
do: |-
  바텀시트의 CTA는 반드시 브랜드 컬러로 활용
  인라인으로 작게 사용 시 2nd depth로 사용 가능
  땡겨요 등의 별도 브랜드 서비스에서는, 버튼 컬러 변경 가능
dont: SOL 브랜드의 서비스는 두가지 색상 외에 사용 불가
---

# 추가 정보를 적을 수 있는 곳

자유롭게 메모를 마크다운으로 할 수 있는 곳
